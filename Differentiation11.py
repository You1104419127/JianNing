...
distance_plate1 = [i for i in range(100)]
distance_plate2 = [j for j in range(100)]
...
#      ATTENTION: The Special case at the last column
#              T % n = 0, at this case, T is at the most right side, it always less than C % n and
#                will always be misunderstood by program that T on the LEFT. Thus, I define it as
#                T % n = n just to make sure that computer knows it is on the right side definitely.
#           
...
#$$$       k = k / (10**(14))          # use this to make it simpler and more visualiable
...
# Do for charge 1: 
potential1 = [i for i in range(100)]
electric_field1 = [i for i in range(100)]
...
# Do for charge 2:
potential2 = [i for i in range(100)]
electric_field2 = [i for i in range(100)]
...
electric_field1[54] = 0  # the potential of the charge itself is not defined so zero here I think, thus
                         #   its electric field is also zero.
electric_field1[55] = 0
electric_field2[54] = 0
electric_field2[55] = 0

potential1 = np.array(potential1)
potential2 = np.array(potential2)

potential = potential1 + potential2
potential = np.reshape(potential,(10,10)) # reshape it into a 10 by 10 matrix
